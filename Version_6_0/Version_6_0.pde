//By: Zeyad Ghulam, Kieran Haliday (Immortal Cells) 2015/2016
...
String sMsg, sMsg2, myText = "", sName;
float fNumStaticBalls, fNumMovingBalls, fNumVirusBalls, CameraZoom, gridSize;
int EASYSCORE, MEDIUMSCORE, HARDSCORE, playerScore, highplayerScore; 
int gameState = 0, nWinEasyScore, nWinMediumScore, nWinHardScore;
...
void draw() {
  ...
  if (gameState == 2) {
    ...
    //Player vs. Coloured Virus
    for (int h = 0; h < alVirusCells.size(); h++) {
      if (bEasy == false && bMedium == false && bHard == true) {
        isPlayerHitColouredVirus(PlayerCell, alVirusCells.get(h));
      }
    }
    //Virus vs. Moving
...
void isMovingHitStatic(MovingB Moving, StaticB Static) {
  ...
  if (distance < 3) {
    Moving.fDiameter = Moving.fDiameter + (Static.nDiameter * 2.5);
    alStaticCells.remove(Static);
  }
}
...
//Only for level medium (The moving cells go away)
void isPlayerHitMoving2(PlayerB PlayerCell, MovingB Moving) {
  if (PlayerCell.fDiameter > Moving.fDiameter) {
    float collide;
    collide = dist(CameraZoom*(Moving.fX - PlayerCell.fX), CameraZoom*(Moving.fY - PlayerCell.fY), (0 - PlayerCell.fX)/CameraZoom + width/2, (0-PlayerCell.fY)/CameraZoom + height/2);
    if (collide >= PlayerCell.fDiameter * 1.5 && collide <= PlayerCell.fDiameter * 1.4 || collide >= PlayerCell.fDiameter * 1.4 && collide <= PlayerCell.fDiameter * 1.5) {
      Moving.fDx = -Moving.fDx;
      Moving.fDy = -Moving.fDy;
    } else if (collide > PlayerCell.fDiameter * 1.4) {
      Moving.fDx = Moving.fDx;
      Moving.fDy = Moving.fDy;
    }
...
//Collision with Player vs. Coloured Virus
void isPlayerHitColouredVirus(PlayerB Player, VirusB Virus) {
  noStroke();
  float collide;
  collide = dist(CameraZoom*(Virus.fX - Player.fX), CameraZoom*(Virus.fY - Player.fY), (0 - Player.fX)/CameraZoom + width/2, (0-Player.fY)/CameraZoom + height/2);
  if (collide < Player.fDiameter*2) {
...
void playerScore() {
  ...
  rectMode(CORNER);
  fill(255);
  rect(width/68 - 5, height - (height/15), 155, 40);
  fill(0);
  text("Score : " + playerScore, width/14, height - (height/30));
  fill(255);
  //Player's Highscore
  rect(width/52 - 10, height/45, 245, 40);
  fill(0);
...
void time() {
  textAlign(CENTER);
  fill(255);
  rect(width/2 + (width/3) + (width/10) - 13, height/30, 87, 35);
  fill(0);
  text(nf(tm.minute(), 2)+":"+nf(tm.second(), 2), width - 55, height/16);
  tm.stop();
}
//Displays the levels in the TOP MIDDLE
void displayLevel() {
  if (bEasy == true && bMedium ==false && bHard == false) {
    ...
    fill(255);
    rect(width/2 - (width/10) - 15, height/45, 285, 47);
    textSize(40);
    fill(0, 0, 255);
    ...
    fill(255);
    rect(width/2 - (width/10), height/45 + 5, 260, 40);
    textSize(40);
    fill(0, 0, 255);
