...
void isHit(PlayerB PlayerCell, StaticB Static) {
  ...
  if (collide < PlayerCell.fDiameter/2) {// the 2.5 controls the distance between the static ball and the player's ball, increase to attract slower, decrease to attract faster.
    ...
    PlayerCell.fDiameter = PlayerCell.fDiameter +  Static.fDiameter/Static.fDiameter;  // Now the PlayerCell.fDiameter increases by ones. Mass increases by ones.
    println(PlayerCell.fDiameter);
    CameraZoom = CameraZoom - Static.fDiameter/500;  // the 500 controls the grid size, is playercell is bigger, the grid should shrinks
