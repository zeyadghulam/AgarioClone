...
int isHitGridX() { 
  if (CameraZoom*(0-PlayerCell.fX) > (width/2-PlayerCell.nDiameter/2)) { //Checks left
    return 1;
  } else if (CameraZoom*(gridSize-PlayerCell.fX) < (width/2+PlayerCell.nDiameter/1.5)) { //Checks right
    return 2;
  }
  return 0;
}
...
int isHitGridY() {
  if (CameraZoom*(0-PlayerCell.fY) > (height/2-PlayerCell.nDiameter/2)) { // Checks top
    return 1;
  } else if (CameraZoom*(gridSize-PlayerCell.fY) < (height/2+PlayerCell.nDiameter/1.5)) { // checks bottom
    return 2;
  } 
  return 0;
}